...
char *MD5::make_digest(MD5Hex_t &md5hex, int len) {
  ...
  for (int i = 0; i < len; i++) {
    md5hex.digest[(i * 2)    ] = hexits[md5hex.hash[i] >> 4];
    md5hex.digest[(i * 2) + 1] = hexits[md5hex.hash[i] & 0x0F];
  }
  md5hex.digest[len * 2] = '\0';
...
#define SET(n) \
  (ctx->block[(n)] = \
     (MD5_u32plus)ptr[(n)*4] | ((MD5_u32plus)ptr[(n)*4 + 1] << 8) | ((MD5_u32plus)ptr[(n)*4 + 2] << 16) | ((MD5_u32plus)ptr[(n)*4 + 3] << 24))
#define GET(n) \
  (ctx->block[(n)])
#endif
...
unsigned char *MD5::make_hash(const char *arg, MD5Hex_t &md5hex) {
  MD5_CTX context;
  MD5Init(&context);
  MD5Update(&context, arg, strlen(arg));
  MD5Final(md5hex.hash, &context);
  return md5hex.hash;
}